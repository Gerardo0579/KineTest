...
json.array! @activity_log do |act|
    ...
    json.start_time Time.strptime(act.start_time.to_s, "%Y-%m-%d %k:%M:%S").iso8601
    json.stop_time Time.strptime(act.stop_time.to_s, "%Y-%m-%d %k:%M:%S").iso8601 unless (act.stop_time.nil?)
